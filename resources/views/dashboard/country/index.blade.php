                                ...
                                <table class="table table-striped" id="custom_tbl_dt">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="text-align:center;">@lang('dashboard.delivery_cost')</th>
                                            <th style="text-align:center;">@lang('dashboard.min_order')</th>
                                    ...
                                    <tbody>
                                        @if (count($countries) > 0)
                                            @foreach ($countries as $key => $country)
                                                <tr class="gradeX {{ $country['id'] }}">
                                                    <td style="text-align:center;">{{ $key + 1 }}</td>
                                                    <td style="text-align:center;">{{ $country['name_ar'] }}</td>
                                                    <td style="text-align:center;">{{ $country['name_en'] }}</td>
                                                    <td style="text-align:center;">{{ $country['delivery_cost'] }}</td>
                                                    <td style="text-align:center;">{{ $country['min_order'] }}</td>
                                                    <td style="text-align:center;">
                                                        @if ($country['display'] == 1)
                                                            <input class="off cm-toggle" data-id="{{ $country['id'] }}"
                                                                type="checkbox" id="toggle"checked data-plugin="switchery"
                                                                data-color="#81c868" />
                                                        @else
                                                            <input class="on cm-toggle" data-id="{{ $country['id'] }}"
                                                    ...
                                                    <td class="actions">
                                                        <a data-id="{{ $country['id'] }}" class="deletemsg"
...
@push('custom-scripts')
    ...
    <script>
                ...
                buttons: {
                    ...
                    main: {
                        ...
                        callback: function() {
                            ...
                            deleteForm.attr('action', "countries/" + id);
                            deleteForm.submit();
                        }
                    }
                }
        ...
        body.on('change', '.off', function() {
            const id = $(this).attr('data-id');
            swal({
                title: "@lang('dashboard.hideCountry')",
                text: "",
                type: "warning",
                ...
                if (isConfirm) {
                    swal("@lang('dashboard.hiddenCountry')", "", "success");
                    $.ajax({
                        type: "POST",
                        url: "{{ route('countries.switch') }}",
                        data: {
                            _token: "{{ csrf_token() }}",
                            id: id,
                            display: 0
        ...
        body.on('change', '.on', function() {
            const id = $(this).attr('data-id');
            swal({
                title: "@lang('dashboard.showCountry')",
                text: "",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55",
                ...
                if (isConfirm) {
                    swal("@lang('dashboard.showedCountry')", "", "success");
                    $.ajax({
                        type: "POST",
                        url: "{{ route('countries.switch') }}",
                        data: {
