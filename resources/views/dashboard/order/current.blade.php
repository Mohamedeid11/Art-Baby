                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            ...
                            <ol class="breadcrumb">
                                ...
                                <li class="active">@lang('dashboard.currentOrders')</li>
                            </ol>
                        </div>
                    </div>
                                ...
                                <table class="table table-striped" id="custom_tbl_dt">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="text-align:center;">@lang('dashboard.transport')</th>
                                        ...
                                        @if (count($orders) > 0)
                                            @foreach ($orders as $key => $order)
                                                <tr class="gradeX {{ $order['id'] }}">
                                                    ...
                                                    @if($order->deliveryId)
                                                    <td style="text-align:center;">
                                                       <p>{{ $order->deliveryId['orderId'] }} - {{ $order['status'] }}</p>
                                                    </td>
                                                    @else
                                                        <td style="text-align:center;">
                                                            <a type="button" class="btn btn-primary" href="{{ route('transport.index', $order['id']) }}">
                                                                @lang('dashboard.transport')
                                                            </a>
                                                        </td>
                                                    @endif
                                                    <td style="text-align:center;">
                                                        <select class="form-control changStatus"
                                                            ...
                                                            <option {{ $order['status'] == 0 ? 'selected' : '' }}
                                                                value="0">@lang('dashboard.pending')</option>
                                                            <option {{ $order['status'] == 1 ? 'selected' : '' }}
                                                                value="1">@lang('dashboard.preparing')</option>
                                                            <option value="2">@lang('dashboard.delivered')</option>
                                                        </select>
                                                    </td>
                                                    ...
                                                    <td class="actions">
                                                        <a data-id="{{ $order['id'] }}" class="deletemsg"
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h3 class="modal-title" id="exampleModalLabel">@lang('dashboard.orderNo').{{ $order['id'] }}</h3>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                ...
                                @foreach ($order->orderProducts as $key => $orderProduct)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $orderProduct->product['name_' . app()->getLocale()] }}</td>
