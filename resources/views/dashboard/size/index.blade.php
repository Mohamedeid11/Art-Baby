        ...
        <div class="content-page">
            ...
            <div class="content">
                <div class="container">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <h4 class="page-title">@lang('dashboard.sizes')</h4>
                            <ol class="breadcrumb">
                                <li><a href="">@lang('dashboard.sizes')</a></li>
                                    ...
                                    <tbody>
                                        @if (count($sizes) > 0)
                                            @foreach ($sizes as $key => $size)
                                                <tr class="gradeX {{ $size['id'] }}">
                                                    <td style="text-align:center;">{{ $key + 1 }}</td>
                                                    <td style="text-align:center;">{{ $size['name_ar'] }}</td>
                                                    <td style="text-align:center;">{{ $size['name_en'] }}</td>
                                                    <td style="text-align:center;">
                                                        @if ($size['display'] == 1)
                                                            <input class="off cm-toggle" data-id="{{ $size['id'] }}"
                                                                type="checkbox" id="toggle"checked data-plugin="switchery"
                                                                data-color="#81c868" />
                                                        @else
                                                            <input class="on cm-toggle" data-id="{{ $size['id'] }}" type="checkbox"
                                                    ...
                                                    <td class="actions">
                                                        <a data-id="{{ $size['id'] }}" class="deletemsg"
    ...
    <script>
                ...
                buttons: {
                    ...
                    main: {
                        ...
                        callback: function() {
                            ...
                            deleteForm.attr('action', "sizes/" + id);
                            deleteForm.submit();
                        }
                    }
                }
        ...
        body.on('change', '.off', function() {
            ...
            swal({
                ...
                if (isConfirm) {
                    swal("@lang('dashboard.hiddenSize')", "", "success");
                    $.ajax({
                        type: "POST",
                        url: "{{ route('sizes.switch') }}",
                        data: {
                            _token: "{{ csrf_token() }}",
                            id: id,
                            display: 0
        ...
        body.on('change', '.on', function() {
            ...
            swal({
                ...
                if (isConfirm) {
                    swal("@lang('dashboard.showedSize')", "", "success");
                    $.ajax({
                        type: "POST",
                        url: "{{ route('sizes.switch') }}",
                        data: {
