                            ...
                            <div class="table-responsive">
                                <table class="table table-striped" id="custom_tbl_dt">
                                    ...
                                    @if(count($recipes) > 0)
                                        ...
                                        <tr class="gradeX {{ $recipe['id'] }}">
                                            <td style="text-align:center;">{{ $key + 1 }}</td>
                                            <td style="text-align:center;">{{ $recipe['title_ar'] }}</td>
                                            <td style="text-align:center;">{{ $recipe['title_en'] }}</td>
                                            <td style="text-align:center;">
                                                <img src="{{ $recipe['image'] }}" alt="{{ $recipe['title_en'] }}" width="150">
                                            </td>
                                            <td style="text-align:center;">
                                                @if ($recipe['display'] == 1)
                                                <input class="off cm-toggle" data-id="{{ $recipe['id'] }}" type="checkbox" id="toggle"checked data-plugin="switchery" data-color="#81c868"/>
                                                @else
                                                <input class="on cm-toggle" data-id="{{ $recipe['id'] }}" type="checkbox" id="toggle"data-plugin="switchery" data-color="#81c868"/>
                                                @endif
                                            </td>
                                            <td>
                                                <a target="_blank" href="{{ $recipe['link'] }}" class="on-default"><i class="fa fa-link"></i></a>
                                            </td>
                                            <td>
                                                <a href="{{ route('recipes.edit', $recipe['id']) }}" class="on-default"><i class="fa fa-pencil"></i></a>
                                            </td>
                                            <td class="actions">
                                                <a data-id="{{ $recipe['id'] }}" class="deletemsg" id="deleteParent"><i class="fa fa-trash-o"></i></a>
                                            </td>
                                        </tr>
                                        @endforeach
                                    @else
...
@push('custom-scripts')
    ...
    <script>
                ...
                buttons: {
                    ...
                    main: {
                        ...
                        callback: function () {
                            ...
                            deleteForm.attr('action', "recipes/" + id);
                            deleteForm.submit();
                        }
                    }
                }
        ...
        body.on('change', '.off', function () {
            const id = $(this).attr('data-id');
            swal({
                title: "@lang('dashboard.hideRecipe')",
                text: "",
                type: "warning",
                ...
                if (isConfirm) {
                    swal("@lang('dashboard.hiddenRecipe')", "", "success");
                    $.ajax({
                        type: "POST",
                        url: "{{ route('recipes.switch') }}",
                        data: {
                            _token: "{{ csrf_token() }}",
                            id: id,
                            display: 0
        ...
        body.on('change', '.on', function () {
            const id = $(this).attr('data-id');
            swal({
                title: "@lang('dashboard.showRecipe')",
                text: "",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55",
                ...
                if (isConfirm) {
                    swal("@lang('dashboard.showedRecipe')", "", "success");
                    $.ajax({
                        type: "POST",
                        url: "{{ route('recipes.switch') }}",
                        data: {
