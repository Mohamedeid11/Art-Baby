        ...
        <div class="content-page">
            ...
            <div class="content">
                <div class="container">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <h4 class="page-title">@lang('dashboard.regions')</h4>
                            <ol class="breadcrumb">
                                <li><a href="">@lang('dashboard.regions')</a></li>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped" id="custom_tbl_dt">
                                    ...
                                    @if(count($regions) > 0)
                                        ...
                                        <tr class="gradeX {{ $region['id'] }}">
                                            <td style="text-align:center;">{{ $key + 1 }}</td>
                                            <td style="text-align:center;">{{ $region['name_ar'] }}</td>
                                            <td style="text-align:center;">{{ $region['name_en'] }}</td>
                                            <td style="text-align:center;">
                                                @if ($region['display'] == 1)
                                                <input class="off cm-toggle" data-id="{{ $region['id'] }}" type="checkbox" id="toggle"checked data-plugin="switchery" data-color="#81c868"/>
                                                @else
                                                <input class="on cm-toggle" data-id="{{ $region['id'] }}" type="checkbox" id="toggle"data-plugin="switchery" data-color="#81c868"/>
                                                @endif
                                            </td>
                                            <td>
                                                <a href="{{ route('regions.edit', $region['id']) }}" class="on-default"><i class="fa fa-pencil"></i></a>
                                            </td>
                                            <td class="actions">
                                                <a data-id="{{ $region['id'] }}" class="deletemsg" id="deleteParent"><i class="fa fa-trash-o"></i></a>
                                            </td>
                                        </tr>
                                        @endforeach
                                    @else
...
@push('custom-scripts')
    ...
    <script>
                ...
                buttons: {
                    ...
                    main: {
                        ...
                        callback: function () {
                            ...
                            deleteForm.attr('action', "regions/" + id);
                            deleteForm.submit();
                        }
                    }
                }
        ...
        body.on('change', '.off', function () {
            const id = $(this).attr('data-id');
            swal({
                title: "@lang('dashboard.hideRegion')",
                text: "",
                type: "warning",
                ...
                if (isConfirm) {
                    swal("@lang('dashboard.hiddenRegion')", "", "success");
                    $.ajax({
                        type: "POST",
                        url: "{{ route('regions.switch') }}",
                        data: {
                            _token: "{{ csrf_token() }}",
                            id: id,
                            display: 0
        ...
        body.on('change', '.on', function () {
            const id = $(this).attr('data-id');
            swal({
                title: "@lang('dashboard.showRegion')",
                text: "",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55",
                ...
                if (isConfirm) {
                    swal("@lang('dashboard.showedRegion')", "", "success");
                    $.ajax({
                        type: "POST",
                        url: "{{ route('regions.switch') }}",
                        data: {
